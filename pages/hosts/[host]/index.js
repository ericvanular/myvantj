...
export default function Home({ host, id, username, description, avatar_url, banner_url }) {
  ...
  */
    ...
    const { data: paymentMethodsData, error: paymentMethodsError } = useSWR(
		keycloak?.token ? [`http://localhost:5000/api/subscription/payment_method`, keycloak?.token] : null,
		fetchWithToken
	)
			...
			<div className="divide-y divide-gray-200 dark:divide-gray-700">
				<div className="pb-8 space-y-2 md:space-y-5">
                    ...
                    <div className="flex flex-row items-start justify-start">
                        <button onClick={() => setShowFollowModal(true)} className="bg-transparent hover:bg-indigo-600 text-indigo-700 font-semibold hover:text-white py-2 px-4 mx-2 border border-indigo-800 hover:border-transparent rounded">
                            Follow
                        </button>
                        <button onClick={() => setShowTipModal(true)} className="bg-transparent hover:bg-green-600 text-indigo-700 font-semibold hover:text-white py-2 px-4 mx-2 border border-green-800 hover:border-transparent rounded">
                            Tip
                        </button>
                        {/*
                        <Link
