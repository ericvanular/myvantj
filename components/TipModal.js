...
export default function TipModal(props) {
	...
	const handleTip = async (amount) => {
		...
		const response = await fetchWithToken(`http://localhost:5000/app/api/patron/tip/${props.username}`, amount)
		if (response.result) {
			setStatus('success')
			setTimeout(() => {
				props.setOpen(false)
			...
			{keycloak?.authenticated ? (
				<>
					...
					<div className="flex items-center justify-between p-4 border-b border-solid border-blueGray-200 rounded-t">
						<h2 className="text-3xl font-semibold">Tip {props.username}?</h2>
						<button
